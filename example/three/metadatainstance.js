...
function reinstantiateTiles() {
	...
	// create tileset
	// tiles = new TilesRenderer("http://localhost:9003/model/Knv6oMjZ/tileset.json");
	tiles.registerPlugin( new CesiumIonAuthPlugin( { apiToken: params.accessToken, assetId: params.assetId } ) );

	tiles.registerPlugin( new ImplicitTilingPlugin() );
	tiles.registerPlugin( new GLTFExtensionsPlugin( { metadata: true,meshoptDecoder:MeshoptDecoder } ) );
...
function findStructuralMetadata( object ) {

	let current = object;
	while ( current ) {

		if ( current.userData.structuralMetadata ) {
...
// update raycasting to find picked mesh / instance features
function updateMeshFeatureRaycast() {
	...
	const { object, face, point, index, faceIndex, instanceId } = hit;

	// GPU instanced features
	if ( object.isInstancedMesh && instanceId != null ) {

		const { instanceFeatures } = object.userData;
		if ( instanceFeatures ) {

			selectedInfo = {
				object,
				instanceId,
				features: instanceFeatures.getFeatures( instanceId ),
				structuralMetadata: findStructuralMetadata( object ),
				instanceFeatures,
			};
		...
		meshFeatures.getFeaturesAsync( faceIndex, barycoord )
			.then( features => {
				...
				selectedInfo = {
					...
					barycoord,
					meshFeatures,
					structuralMetadata: object.userData.structuralMetadata ?? findStructuralMetadata( object ),
				};
...
function appendStructuralMetadata( structuralMetadata, triangle, barycoord, index = null, tableIndices = null, features = null ) {
	...
	if ( triangle !== null && barycoord !== null ) {
...
function updateMetaDataDisplay() {
	...
	if ( selectedInfo ) {
		...
		instanceId = selectedInfo.instanceId ?? null;
		meshFeatures = selectedInfo.instanceFeatures ?? selectedInfo.meshFeatures;
		structuralMetadata = selectedInfo.structuralMetadata;
		features = selectedInfo.features;

	}

	if ( meshFeatures !== null && features !== null && instanceId !== null ) {

		meshFeaturesEl.innerText = 'EXT_INSTANCE_FEATURES\n\n';
		meshFeaturesEl.innerText += `instanceId     : ${ instanceId }\n`;
		...
		if ( structuralMetadata !== null ) {
			...
			appendStructuralMetadata( structuralMetadata, null, null, null, tableIndices, features );
	...
	} else if ( meshFeatures !== null && features !== null ) {

		const { index, faceIndex, barycoord } = selectedInfo;
		...
		meshFeaturesEl.innerText = 'EXT_INSTANCE_FEATURES\n\n';
		meshFeaturesEl.innerText += 'instanceId     : -\n';
